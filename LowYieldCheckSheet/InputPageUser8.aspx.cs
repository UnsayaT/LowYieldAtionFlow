...
{
    public partial class InputPageUser8_Test : System.Web.UI.Page
    {
        ...
        public void SetValue()
        {
            ...
            {
                if (SelectS14A1.SelectedIndex == 1)
                {
                    gc.Status = "7";
                }
                else
            ...
            if (tbl.Rows.Count == 1)
            {
                foreach (DataRow row in tbl.Rows)
                {
                    gc.QCIncharge_Sign = row["name"].ToString(); //get name of user
                    gc.QCApprove_Sign = row["name"].ToString(); //get name of user
                }
            }
            else
            {
                gc.QCIncharge_Sign = gc.User;
                gc.QCApprove_Sign = gc.User;
            }

            gc.QCIncharge_SignDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"); // case sensitive
        ...
        public void ConfirmBtnOnClick(object sender, EventArgs e)
        {
            ...
            SetValue();
            dba.UpdatePageUser8(gc.Process, gc.HandlerNo, gc.LotNo, gc.Status, gc.RequestAQIS15, gc.StopShipObjDevice, gc.StopLabel, gc.FTRetestGood,
                gc.FTRetestNG, gc.ObjectScope, gc.StopRecallShipment, gc.StopShipmentPKG, gc.Assy, gc.QCJudgement,
                gc.QCIncharge_ID, gc.QCIncharge_Sign, gc.QCIncharge_SignDate, gc.QCApprove_ID, gc.QCApprove_Sign, gc.QCApprove_Date);
