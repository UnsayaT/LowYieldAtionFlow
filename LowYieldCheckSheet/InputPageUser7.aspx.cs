...
{
    public partial class InputPageUser7_Test1 : System.Web.UI.Page
    {
        ...
        public void SetValue()
        {
            ...
            if (tbl.Rows.Count == 1)
            {
                foreach (DataRow row in tbl.Rows)
                {
                    gc.PDMgr_Sign = row["name"].ToString(); //get name of user
                }
            }
            else
            {
                gc.PDMgr_Sign = gc.User;
            }
            gc.PDMgr_SignDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"); // case sensitive
        ...
        {
            SetValue();
            dba.UpdatePageUser7(gc.Process, gc.HandlerNo, gc.LotNo, gc.Status, gc.PDMgr_ID, gc.PDMgr_Sign, gc.PDMgr_SignDate);

            //ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Save Done')", true);
            Session["Status"] = "7";
            //strUrl = "~/PageMonitoring.aspx";
            //Response.Redirect(strUrl);

            strUrl = "~/PageMonitoring_PDMgr.aspx?userId=" + Request.Params["User"];
            Response.Redirect(strUrl);
        }
